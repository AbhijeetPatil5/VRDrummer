...
**Purpose:** Project architecture reference and implementation guide
...
VRDrummer is an app designed to teaches novice drummers using mixed reality. The player wears 
a VR headset, sees their real drumset through passthrough, and receives circular glow cues
on each physical pad. Strikes are captured via real drumsticks hits (USB MIDI — not VR
controllers). Timing feedback and song charts come **last**, after perception works reliably.


### Hardware

The hardware configuration for this project is listed below; however, the application is designed to function with any compatible hardware substitute.
...
| GPU               | RTX 4080 Super                                 |
| OS                | Windows 11                                     |
| VR Headset        | Meta Quest 3S, developer mode                  |
| VR Headset OS     | Horizon OS v74+                                |
| Link (MR display) | Wired USB-C ≥ 2 Gbps                           |
| Drumset           | Simmons Titan 50 B-EX (USB MIDI)               |
...
| Phase    | Deliverable                                                                |
| -------- | -------------------------------------------------------------------------- |
| **1a**   | Establish connection between drumset and PC                                |
| **1b**   | Create OpenXR instance on the PC                                           |
| **2a**   | USB camera stream (bootstrap: H.264, CPU decode)                           |
| **2b**   | NVDEC zero-copy decode on PC                                               |
| **2c**   | Stereo dual-port + metadata + **stereo calibration harness**               |
| **2d**   | Wi-Fi WebRTC stream (after USB path proven)                                |
| **3–8**  | Perception MVP: kit detect → pad ID → EKF → MR glows → robustness → replay |
| **9–13** | Gameplay **(last):** MIDI merge, charts, scoring, `.mid` / `.pdf`          |


### Perception MVP (First "It Works" Moment)

- Live passthrough camera frames decoded on PC
- Drum kit localised in frame; all drum pads identified
- Stable pad poses via estimation
...
Six stages where latency accumulates:
...
[5 Infer+Estimate] TensorRT YOLO11-seg → ByteTrack → stereo triangulation → Estimation
...
flowchart TB
    subgraph quest [Quest3S]
        ...
        PCA_R[PCA_right_1280x960_or_1280x1280]
...
| ------------------------------------------------------------------- | ----------------------------------------------------- | ----------- |
| [RtMidi 6.0.0](https://github.com/thestk/rtmidi/releases/tag/6.0.0) | `RtMidiIn`, `openPort`, `setCallback`                 | Done           |
...
| ------ | --------------------------------------------- | ----------------------------------------------------------------------- | ---- | -------- |
| 1a     | MIDI                                          | RtMidi 14-pad map                                                       | —    | Done     |
| 1b     | OpenXR probe                                  | Instance + passthrough proc load                                        | —    | Partial  |
| 2a     | USB camera stream (bootstrap)                 | Left eye, H.264, CPU decode, ffplay verify                              | 1    | 3–5 wk   |
| 2b     | NVDEC + zero-copy                             | NVDEC → NV12 → cv::cuda BGR; ORT IOBinding (Tier 1b)                    | 2    | 2–3 wk   |
| 2c     | Stereo dual-port + metadata + calibration     | Both eyes, timestamp sync, intrinsics side channel, `stereo_calib.json` | 2    | 2–3 wk   |
| 2d     | Wi-Fi WebRTC stream                           | QuestCameraKit → libdatachannel; CBP + no jitter buffer                 | 3    | 3–5 wk   |
| 3      | Kit detection                                 | OpenCV contours or YOLO `drum_kit`                                      | 1    | 2–4 wk   |
| 4      | Pad ID 14-class seg                           | YOLO11n-seg ONNX; CVAT labels                                           | 1→2  | 4–8 wk   |
| 4b     | TensorRT engine                               | FP16 then INT8 with calibration                                         | 2    | 1–2 wk   |
| 4c     | ByteTrack integration                         | Hungarian + IoU; filter spurious detections                             | 2    | 1–2 wk   |
| 5      | EKF + stereo fusion                           | Eigen3; `xrLocateSpace`; stereo triangulation                           | 2    | 3–5 wk   |
| 6      | MR glow overlays                              | XrPassthrough + D3D11; `XR_FB_color_space`                              | 2    | 3–5 wk   |
| 7      | Robustness                                    | CLAHE, EKF noise tuning, dropout recovery                               | 2    | 3–5 wk   |
| 8      | Record/replay tooling                         | ImGui replay; optional QuestRealityCapture log format                   | 2    | 2–3 wk   |
| 9      | MIDI merge                                    | RtMidi lock-free queue in unified app                                   | 4    | 1–2 wk   |
| 10     | Chart + predictive glow                       | Hand tracking wrist prior into EKF                                      | 4    | 2–3 wk   |
| 11     | Scoring flashes                               | ±25 ms green/red/purple                                                 | 4    | 1–2 wk   |
| 12     | Latency lab                                   | E2E ≤ 30 ms strike-to-flash; ImGui HUD                                  | 4    | 1–2 wk   |
| 13     | `.mid` / `.pdf` import                        | libsmf / PDF viewer                                                     | 4    | 4–8 wk   |
...
# Version 5.1 Review Addendum (Architecture Audit)

## Structural Corrections Applied

### Milestone Naming Normalization
...
Avoid skipped labels and ensure every referenced milestone exists in the roadmap.

### Recommended New Milestone

#### M1e — Dataset, Replay and Benchmark Infrastructure
...
This should be completed before major perception development because it dramatically improves debugging, benchmarking, and regression testing.
...
Maintain a transform tree and document all frame conversions.
...
Allows quantitative comparison of estimation approaches.

### Unified Time Domain

Create a single timestamp standard spanning:
...
All latency calculations should derive from this system.
...
- Decode failures
- Tracker confidence
- Stereo synchronization error
- End-to-end latency
- Queue depths

### Simulation Mode

Support operation without hardware using:

- Recorded video
- Synthetic detections
- Synthetic MIDI

This significantly accelerates development and testing.
...
Centralize tuning parameters instead of hardcoding them.

---

## Documentation Cleanup Recommendations

1. Keep acceptance tests immediately adjacent to milestone definitions.
2. Keep open questions attached to their owning milestone.
3. Maintain a single source of truth for milestone numbering.
4. Reserve “Tier” for capability level only.
5. Reserve “Milestone” for implementation progress only.
6. Expand the glossary whenever a new acronym appears.
