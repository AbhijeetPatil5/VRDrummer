...
| # | Question | Milestone |
|---|----------|-----------|
| 1 | Does 1280×1280 PCA resolution cover all 14 pads from seated position? | M4 |
| 2 | Does the companion streaming app survive while wired link is active on current headset firmware? | M5 |
| 3 | HEVC HW encode on the headset's SoC in the companion app fork? | M6 |
| 4 | Link passthrough beta tab missing on current firmware — registry workaround? | M3 |
| 5 | Accessory-mode (AoA) direct bulk USB on current headset firmware? | Research |
| 6 | libdatachannel jitter buffer disable — which `rtc::Configuration` field? | M33 |
| 7 | Classical CV vs YOLO for v1 kit ROI baseline — which generalizes better? | M11 |
| 8 | StereoBM vs StereoSGM quality on drum pad surfaces (reflective rubber)? | M15 |
| 9 | Correct color space constant for the current headset (`XR_COLOR_SPACE_RIFT_CV1_FB`)? | M17 |
| 10 | Audio library choice: miniaudio vs SDL_mixer vs FMOD? | M31 |
| 11 | `.mid` parser: libsmf vs craigsapp/midifile vs custom? | M20 |
| 12 | VR UI toolkit: custom D3D11 panels vs ImGui in VR vs existing framework? | M27 |
| 13 | `.pdf` drum chart display — worth the complexity or cut from MVP? | Post-MVP |
| 14 | D3D11/CUDA shared texture handle for glow rendering — feasible and worth it? | M34 |

---

## 13. Glossary and Document History
...
| 6.0 | Jun 2026 | Unified phase/milestone/tier nomenclature; integrated replay, frames, abstractions, config, health, simulation; perception-focused |
| **7.0** | **Jun 2026** | **Ground-up rewrite.** Full product architecture (MR platform, perception, MIDI, music engine, gameplay, scoring, UI, audio, content). Layered architecture (Platform → Input → Processing → State → Logic → Output → Data). Performance and concurrency architecture (threads, lock-free queues, zero-copy GPU, CUDA pipelining, interpolation). 10-phase roadmap with 38 flat milestones covering full product lifetime. Developer learning path (10 tracks). Implementation reference with organized code snippets. Generalized hardware references. Design Constraints dissolved into technical notes. |
